...
@section('content')
    ...
    <div style="margin-left:220px; padding:24px; flex:1; background:#f0fdf4;">
        ...
        <a href="{{ route('teacher.modules') }}" style="font-size:13px; color:#064e3b; text-decoration:none;">
            <i class="fa fa-arrow-left"></i> Back
        </a>

        <h1 style="font-size:22px; font-weight:600; color:#064e3b; margin:12px 0 4px;">
            <span style="font-size:13px; background:#d1fae5; color:#059669; padding:2px 8px; border-radius:5px; margin-right:8px;">{{ $module->module_code }}</span>
            {{ $module->title }}
        </h1>
        ...
        <div style="display:flex; gap:8px; margin-bottom:20px; border-bottom:1px solid #e5e7eb;">
            <button onclick="showTab('topics')" id="tab-topics" class="module-tab active-tab" style="padding:10px 16px; border:none; background:none; font-size:14px; cursor:pointer; border-bottom:2px solid #059669; color:#059669; font-weight:600;">Topics / Lecture Notes</button>
        ...
        <div id="content-topics" class="module-tab-content">
            <button onclick="openAddTopicModal()" style="background:#059669;color:#fff;border:none;border-radius:8px;padding:8px 16px;font-size:13px;font-weight:700;cursor:pointer; margin-bottom:16px;">
                + Add Topic
            </button>
                ...
                @forelse ($module->topics as $topic)
                    <div style="display:flex; justify-content:space-between; align-items:flex-start; padding:14px 0; border-bottom:1px solid #f3f4f6;">
                        <div>
                            ...
                            @if ($topic->file_path)
                                <a href="{{ asset('storage/' . $topic->file_path) }}" target="_blank" style="font-size:12px; color:#059669; text-decoration:none;">
                                    <i class="fa fa-file-text-o"></i> {{ $topic->original_name }}
                                </a>
                            @endif
                        </div>
                        <div>
                            <form id="delete-topic-form-{{ $topic->id }}" action="{{ route('teacher.topics.destroy', $topic) }}" method="POST" style="display:inline;">
                                @csrf
                                @method('DELETE')
        ...
        <div id="content-assignments" class="module-tab-content" style="display:none;">
            <button onclick="openAddAssignmentModal()" style="background:#059669;color:#fff;border:none;border-radius:8px;padding:8px 16px;font-size:13px;font-weight:700;cursor:pointer; margin-bottom:16px;">
                + Add Assignment
            </button>
                    ...
                    <div style="display:flex; justify-content:space-between; align-items:flex-start; padding:14px 0; border-bottom:1px solid #f3f4f6;">
                        <div>
                            ...
                            @if ($assignment->file_path)
                                <br><a href="{{ asset('storage/' . $assignment->file_path) }}" target="_blank" style="font-size:12px; color:#059669; text-decoration:none;">
                        ...
                        <div>
                            <form id="delete-assignment-form-{{ $assignment->id }}" action="{{ route('teacher.assignments.destroy', $assignment) }}" method="POST" style="display:inline;">
                                @csrf
                                @method('DELETE')
        ...
        <div id="content-exams" class="module-tab-content" style="display:none;">
            <button onclick="openAddExamModal()" style="background:#059669;color:#fff;border:none;border-radius:8px;padding:8px 16px;font-size:13px;font-weight:700;cursor:pointer; margin-bottom:16px;">
                + Add Exam
            </button>
                ...
                <div style="background:white; border:1px solid #e5e7eb; border-radius:12px; padding:20px; margin-bottom:16px;">
                    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px;">
                        ...
                        <div>
                            @if ($exam->status === 'draft')
                                <button onclick="openAddQuestionModal({{ $exam->id }})" style="background:#d1fae5; color:#059669; border:none; border-radius:6px; padding:6px 12px; font-size:12px; cursor:pointer; margin-right:6px;">
                                    + Add Question
                                </button>
                                <form id="publish-exam-form-{{ $exam->id }}" action="{{ route('teacher.exams.publish', $exam) }}" method="POST" style="display:inline;">
                                    @csrf
                            ...
                            @else
                                <a href="{{ route('teacher.exams.results', $exam) }}" style="background:#d1fae5; color:#059669; border:none; border-radius:6px; padding:6px 12px; font-size:12px; text-decoration:none; margin-right:6px;">
                                    <i class="fa fa-bar-chart"></i> View Results
                                </a>
                            @endif
                            <form id="delete-exam-form-{{ $exam->id }}" action="{{ route('teacher.exams.destroy', $exam) }}" method="POST" style="display:inline;">
                                @csrf
                                @method('DELETE')
                    ...
                    @foreach ($exam->questions as $i => $question)
                        <div style="padding:12px 0; border-top:1px solid #f3f4f6;">
                            ...
                            @if ($exam->status === 'draft')
                                <form id="delete-question-form-{{ $question->id }}" action="{{ route('teacher.questions.destroy', $question) }}" method="POST" style="display:inline;">
                                    @csrf
                                    @method('DELETE')
    ...
    <div id="addTopicModal" style="display:none;position:fixed;inset:0;background:rgba(27,27,51,0.4);z-index:50;align-items:center;justify-content:center;">
        <div style="background:#fff;border-radius:16px;padding:32px;width:100%;max-width:400px;">
            ...
            <form method="POST" action="{{ route('teacher.topics.store', $module) }}" enctype="multipart/form-data">
                @csrf
                ...
                <div style="display:flex;gap:10px;">
                    ...
                    <button type="submit"
                            style="flex:1;padding:11px;border-radius:8px;border:none;background:#059669;color:#fff;font-weight:700;cursor:pointer;">Add topic</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div id="addAssignmentModal" style="display:none;position:fixed;inset:0;background:rgba(27,27,51,0.4);z-index:50;align-items:center;justify-content:center;">
        <div style="background:#fff;border-radius:16px;padding:32px;width:100%;max-width:420px;max-height:90vh;overflow-y:auto;">
            ...
            <form method="POST" action="{{ route('teacher.assignments.store', $module) }}" enctype="multipart/form-data">
                @csrf
                ...
                <div style="display:flex;gap:10px;">
                    <button type="button" onclick="closeModal('addAssignmentModal')"
                            style="flex:1;padding:11px;border-radius:8px;border:1.5px solid #E4E4EF;background:#fff;cursor:pointer;">Cancel</button>
                    <button type="submit"
                            style="flex:1;padding:11px;border-radius:8px;border:none;background:#059669;color:#fff;font-weight:700;cursor:pointer;">Add assignment</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div id="addExamModal" style="display:none;position:fixed;inset:0;background:rgba(27,27,51,0.4);z-index:50;align-items:center;justify-content:center;">
        <div style="background:#fff;border-radius:16px;padding:32px;width:100%;max-width:400px;">
            ...
            <form method="POST" action="{{ route('teacher.exams.store', $module) }}">
                @csrf
                ...
                <div style="display:flex;gap:10px;">
                    <button type="button" onclick="closeModal('addExamModal')"
                            style="flex:1;padding:11px;border-radius:8px;border:1.5px solid #E4E4EF;background:#fff;cursor:pointer;">Cancel</button>
                    <button type="submit"
                            style="flex:1;padding:11px;border-radius:8px;border:none;background:#059669;color:#fff;font-weight:700;cursor:pointer;">Create exam</button>
                </div>
            </form>
        </div>
    </div>
            ...
            <form id="add-question-form" method="POST">
                ...
                <div style="display:flex;gap:10px;">
                    <button type="button" onclick="closeModal('addQuestionModal')"
                            style="flex:1;padding:11px;border-radius:8px;border:1.5px solid #E4E4EF;background:#fff;cursor:pointer;">Cancel</button>
                    <button type="submit"
                            style="flex:1;padding:11px;border-radius:8px;border:none;background:#059669;color:#fff;font-weight:700;cursor:pointer;">Add question</button>
                </div>
            </form>
        </div>
    </div>
